...
# Function to retrieve data from API based on kode regency
def get_api_data(kode_district):
    # API URL Template
    url = "https://webapi.bps.go.id/v1/api/interoperabilitas/datasource/simdasi/id/28/parent/{}/key/7342305c0d7d5f9fe5a2f0e4c7d08e57/"
    api_base_url = url.format(kode_district)
    ...
    if response.status_code == 200:
        ...
        if status == 'OK' and data_availability == 'available':
            ...
            try:
                ...
                cursor.execute(f'''
                    CREATE TABLE IF NOT EXISTS {schema_name}.district_SIMDASI (
                        lingkup TEXT,
                        versi TEXT,
                        induk INTEGER,
                        wilayah TEXT,
                ...
                for entry in selected_data:
                    cursor.execute(f'''
                        INSERT INTO {schema_name}.district_SIMDASI (lingkup, versi, induk, wilayah, kode, nama, satuan_lingkungan, satuan_lingkungan_en)
...
def process_all_district():
    try:
        # Establish a connection to the PostgreSQL database
        cursor,conn = connDB()
        # Select all provinsi from the database
        cursor.execute('SELECT kode FROM "statistik_bps_simdasi"."regency_SIMDASI"')
        kode_district_list = cursor.fetchall()

        for kode_district in kode_district_list:
            # Extract the actual value from the tuple
            kode_district_value = kode_district[0]

            # Fetch data from the API for the current kode provinsi
            api_data = get_api_data(kode_district_value)

            if api_data:
                # Process the API data as needed
                print(f"Data for kode_district {kode_district_value}:")
                print(api_data)
                print("\n")
